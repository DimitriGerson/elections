...
class ControleurBDSqlite(object):
    ...
    def __init__(self,mabase):
        """Cette methode d'initialisation crée la base de donnée met en place la connection et et le curseur"""
        try:
            self.nom = mabase + ".db"
            """Lancement de la methode ouvrir connexion qui ne prend pas de paramètre."""
            self.monTableauConnecteur=self.ouvrirConnexion()
            print("Creation du connecteur et du curseur")
            return self.monTableauConnecteur
        except:
            print("Une erreur est survenue nous ne sommes pas en mesure de concevoir votre base de données.")
        finally:
